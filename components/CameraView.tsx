  ...
  return (
    <div className={`relative h-screen w-full bg-slate-950 overflow-hidden flex flex-col justify-between ${isAr ? 'font-arabic' : 'font-sans'}`} dir={isAr ? 'rtl' : 'ltr'}>
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-8">
        <div className="relative w-full aspect-[3/4] max-w-sm border border-cyan-500/30 rounded-lg">
          {/* Corners */}
          ...
          <div className="absolute bottom-4 right-4 text-[10px] font-mono-tech text-cyan-300/80">
            ISO: AUTO <br/> W: 4500K
          </div>

          {/* Prompt Text */}
          <div className="absolute -bottom-12 w-full text-center">
             <p className="text-sm font-medium text-white tracking-widest uppercase text-shadow-glow animate-pulse">
               {t.instruction}
             </p>
          </div>
        </div>
      ...
      {/* BOTTOM CONTROLS */}
      <div className="relative z-10 w-full pb-10 pt-4 flex items-center justify-center gap-8 bg-gradient-to-t from-slate-950 via-slate-900/80 to-transparent">
        ...
        <div className="flex flex-col items-center gap-2">
            <button 
                ...
                className="w-14 h-14 rounded-full bg-slate-800/80 border border-cyan-500/30 flex items-center justify-center hover:bg-cyan-900/40 transition-all active:scale-95"
            >
                <svg className="w-6 h-6 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                </svg>
            </button>
            <span className="text-[10px] font-mono-tech text-cyan-600 tracking-widest">{t.upload}</span>
        </div>
